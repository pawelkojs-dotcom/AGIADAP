...
class ArchitectureSearcher:
    ...
    def __init__(self, evaluator: HybridEvaluator, enable_safety_phase2: bool = False):
        ...
        self.enable_safety = enable_safety_phase2
    ...
    def generate_intagi_guided_config(self) -> Dict[str, Any]:
        ...
        """
        # Use hardcoded validated ranges from INTAGI constraints
        return {
            'n_layers': random.choice([5, 6]),  # Validated minimum is 5
            'hidden_dim': random.choice([256, 512]),
            'theta': random.uniform(0.10, 0.15),  # Validated range
            'gamma': random.uniform(0.08, 0.12),  # Validated range
        }
    ...
    def search(
        ...
        while trials < max_trials and successes < target_successes:
            ...
            # Validate with safety (if enabled)
            if self.enable_safety:
                try:
                    arch = Architecture(
                        name=f"trial_{trials}",
                        type="INTAGI_A0",
                        theta=config['theta'],
                        gamma=config['gamma'],
                        n_layers=config['n_layers']
                    )
                    self.safety.validate_architecture(arch)
                except Exception as e:
                    # Safety violation - skip this config
                    print(f"  [Trial {trials}] Safety violation: {e}")
                    continue
...
def compare_strategies(
    ...
    enable_safety_phase2: bool = False
    ...
    Args:
        ...
        enable_safety_phase2: If True, enable Safety Phase 2 (H5-medium)
    ...
    print(f"Safety Phase 2: {'ENABLED' if enable_safety_phase2 else 'disabled'}")
    print("="*60 + "\n")
    
    # Create evaluator
    evaluator = HybridEvaluator(use_api=use_real_api)
    
    # Create searcher with safety
    searcher = ArchitectureSearcher(evaluator, enable_safety_phase2=enable_safety_phase2)
...
def analyze_results(
    ...
    print("\n[IMPROVEMENTS]")
    print(f"  Success rate improvement: {sr_improvement:.2f}Ã—")
    print(f"  Trials reduction: {trials_improvement:.2f}Ã—")
    print(f"  COMBINED SPEEDUP: {combined_speedup:.2f}Ã—")
    print(f"  Cost reduction: {analysis['improvements']['cost_reduction_ratio']:.2f}Ã—")
    ...
    print("\n[VALIDATION]")
    print(f"  Theoretical speedup (from constraints): {theoretical_speedup:.0f}Ã—")
    print(f"  Empirical speedup (measured): {combined_speedup:.2f}Ã—")
...
def main():
    ...
    enable_phase2 = '--safety-phase2' in sys.argv
    n_trials = 50  # Default
    ...
    unconstrained, intagi = compare_strategies(
        ...
        enable_safety_phase2=enable_phase2
    )
    ...
    print("\nâœ… TRL 3.2 Speedup Demonstration COMPLETE!")
