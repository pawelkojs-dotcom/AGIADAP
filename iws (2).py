...
class IntentionalWorldState:
    ...
    def compute_global_coherence(self) -> float:
        """Compute σ_coh across all layers (handles different dimensions)"""
        ...
        for i in range(N):
            for j in range(i + 1, N):
                ...
                v1_padded = np.zeros(max_dim)
                v2_padded = np.zeros(max_dim)
                v1_padded[:len(v1)] = v1
                v2_padded[:len(v2)] = v2
                
                denom = (np.linalg.norm(v1_padded) * np.linalg.norm(v2_padded) + 1e-8)
                if denom > 0:
                    sim = float(np.dot(v1_padded, v2_padded) / denom)
                    total += sim
                    count += 1
