...
class UnifiedTaskManager:
    ...
    def step(self, tasks: List[Task], context: Dict[str, Any]):
        ...
        # FIX Sprint 2.5.2.1: In test mode (DummyLLM), embeddings are random
        # DO NOT update sigma from embeddings - preserve aligned initialization
        # Sigma will evolve ONLY through Langevin dynamics (gradient + noise)
        # This is correct behavior for testing gradient fixes!
        # (In production with real LLM, embeddings would be meaningful)
        
        # L3: Semantic
        semantic_context, I_ratio = self.L3.process(task_embeddings, tasks)
        
        # FIX Sprint 2.5.2.1: Don't update from random embeddings
        # Sigma evolves through Langevin dynamics only
        ...
        # FIX Sprint 2.5.2.1: Don't add random noise here
        # Sigma evolves through Langevin dynamics only (later in this step)
        ...
        self.iws.current_mode = mode  # mode is already a string from decide_mode
        ...
        for layer_name in ['sigma_sensory', 'sigma_semantic', 'sigma_pragmatic']:
            ...
            # NOTE: grad_F is already a FORCE (negative energy gradient),
            # so we DON'T negate it here!
            delta_sigma = (
