...
def run_trl35_campaign(
    ...
    safety_report = evidence['safety_report']
    print("\n" + "="*60)
    print("CAMPAIGN COMPLETE")
    print("="*60)
    print(f"Total runs: {overall.get('total_runs', 0)}")
    print(f"Success rate: {100*overall.get('task_success_rate', 0):.1f}%")
    print(f"Safety violations: {safety_report.get('total_violations', 0)}")
...
def generate_mini_report(evidence: Dict[str, Any]) -> str:
    ...
    overall = results.get('overall', {})
    by_task_type = results.get('by_task_type', {})
...
- **Architecture success rate:** {100*overall.get('arch_success_rate', 0):.1f}%
- **Task success rate:** {100*overall.get('task_success_rate', 0):.1f}%
...
### Phase 1 (H5-lite)
- **Status:** {safety.get('phase1', {}).get('status', 'unknown')}
- **Bounds violations:** {safety.get('phase1', {}).get('bounds_violations', 0)}
- **Recursion violations:** {safety.get('phase1', {}).get('recursion_violations', 0)}

### Phase 2 (H5-medium)
- **Status:** {safety.get('phase2', {}).get('status', 'unknown')}
- **Filesystem violations:** {safety.get('phase2', {}).get('filesystem_violations', 0)}
- **Content violations:** {safety.get('phase2', {}).get('content_violations', 0)}
